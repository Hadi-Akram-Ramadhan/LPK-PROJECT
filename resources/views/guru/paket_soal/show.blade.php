...
<div style="background:#fff;border:1px solid #e2e8f0;border-radius:14px;padding:20px 24px;margin-bottom:20px;display:flex;justify-content:space-between;align-items:center;">
    ...
    <div style="display:flex;gap:10px;">
        ...
        <a href="{{ route('guru.import.index', ['paket' => $paketSoal->id]) }}" style="display:inline-flex;align-items:center;gap:8px;padding:10px 20px;border-radius:10px;font-size:13px;font-weight:600;text-decoration:none;background:#fff;color:#2563eb;border:1.5px solid #2563eb;">
...
<div style="background:#fff;border:1px solid #e2e8f0;border-radius:14px;overflow:hidden;">
        ...
        <tbody>
            ...
            <tr style="border-top:1px solid #f1f5f9;" onmouseover="this.style.background='#f8fafc'" onmouseout="this.style.background=''">
                ...
                <td style="padding:16px 20px;max-width:380px;">
                    ...
                    @if($soal->audio_path)
                    <span style="font-size:11px;color:#2563eb;display:flex;align-items:center;gap:4px;margin-top:4px;">
                        <svg style="width:12px;height:12px;" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.536 8.464a5 5 0 010 7.072M18.364 5.636a9 9 0 010 12.728"/></svg>
                        Ada Audio
                    </span>
                    @endif
                </td>
                <td style="padding:16px 20px;">
                    @if($soal->tipe === 'pilihan_ganda')
                        <span style="background:#dbeafe;color:#2563eb;font-size:11px;font-weight:700;padding:4px 10px;border-radius:20px;">PG</span>
                    ...
                    @elseif($soal->tipe === 'audio')
                        <span style="background:#fef3c7;color:#d97706;font-size:11px;font-weight:700;padding:4px 10px;border-radius:20px;">Audio</span>
                    @else
                        <span style="background:#dcfce7;color:#16a34a;font-size:11px;font-weight:700;padding:4px 10px;border-radius:20px;">Essay</span>
                    @endif
                </td>
                <td style="padding:16px 20px;">
                    @php
                    ...
                    @endphp
                    @if($soal->tipe === 'essay')
                        <span style="font-size:12px;color:#94a3b8;">Esai</span>
                    @elseif($benar)
    ...
    <div style="padding:14px 20px;font-size:13px;color:#94a3b8;border-top:1px solid #f1f5f9;">
        Total {{ $soals->count() }} soal Â· Total poin: {{ $soals->sum('poin') }}
    </div>
</div>
@endif
