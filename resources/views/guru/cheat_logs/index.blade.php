                    ...
                    <td id="row-action-{{ $log->id }}" class="px-6 py-4 whitespace-nowrap text-center text-sm font-medium">
                        ...
                        <div class="flex justify-center space-x-2">
                            ...
                            <form action="{{ route('guru.cheat-logs.approve', $log) }}" method="POST" class="inline ajax-form" data-id="{{ $log->id }}" data-status="approved">
                                ...
                                <button type="submit"
                                    class="inline-flex items-center px-4 py-1.5 border border-transparent text-xs font-medium rounded shadow-sm text-white bg-green-600 hover:bg-green-700 transition-colors">
...
document.addEventListener('DOMContentLoaded', function() {
        ...
        form.addEventListener('submit', function(e) {
            ...
            .then(data => {
                if (data.success) {
                    ...
                    if (actionCell) {
                        actionCell.innerHTML = '<span class="text-slate-300 text-xs">Selesai</span>';
                    }
    ...
    if (window.Echo) {
        console.log("Listening for new cheat logs...");
        window.Echo.channel('cheat-logs')
            .listen('.CheatLogReportedEvent', (e) => {
                console.log("New cheat log received:", e);
                
                // Tampilkan banner notifikasi sederhana
                const bannerId = 'new-log-banner';
                if (!document.getElementById(bannerId)) {
                    const banner = document.createElement('div');
                    banner.id = bannerId;
                    banner.className = 'fixed top-20 right-4 z-50 bg-accent-600 text-white px-6 py-4 rounded-xl shadow-2xl flex items-center space-x-4 animate-bounce cursor-pointer';
                    banner.innerHTML = `
                        <div class="bg-white/20 p-2 rounded-lg">
                            <svg class="h-6 w-6" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"/></svg>
                        </div>
                        <div>
                            <p class="font-bold text-sm">Pelanggaran Baru Terdeteksi!</p>
                            <p class="text-xs opacity-90">Klik di sini untuk memuat ulang data.</p>
                        </div>
                    `;
