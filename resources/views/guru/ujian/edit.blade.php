    ...
    <form action="{{ route('guru.ujian.update', $ujian) }}" method="POST" class="p-8">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
            ...
            <div class="lg:col-span-1 space-y-6">
                ...
                <div>
                    <input type="text" name="judul" id="judul" value="{{ old('judul', $ujian->judul) }}" required maxlength="255" class="shadow-sm focus:ring-accent-500 focus:border-accent-500 block w-full sm:text-sm border-slate-300 rounded-md">
                </div>

                <div>
                    <label for="deskripsi" class="block text-sm font-medium text-slate-700 mb-1">Deksripsi (Opsional)</label>
                    <textarea id="deskripsi" name="deskripsi" rows="3" maxlength="1000" class="shadow-sm focus:ring-accent-500 focus:border-accent-500 block w-full sm:text-sm border-slate-300 rounded-md">{{ old('deskripsi', $ujian->deskripsi) }}</textarea>
                </div>
