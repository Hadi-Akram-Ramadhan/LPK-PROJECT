...
<div class="max-w-xl bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
    <form action="{{ route('admin.kelas.update', $kelas) }}" method="POST" class="p-8">
        @csrf
        @method('PUT')
        ...
        <div class="space-y-6">
            <div>
                ...
                <input type="text" name="nama" id="nama" value="{{ old('nama', $kelas->nama) }}" required autofocus class="shadow-sm focus:ring-primary-500 focus:border-primary-500 block w-full sm:text-sm border-slate-300 rounded-md">
