...
<div class="max-w-xl bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
    <form action="{{ route('admin.kelas.store') }}" method="POST" class="p-8">
        ...
        <div class="space-y-6">
            <div>
                <label for="nama" class="block text-sm font-medium text-slate-700 mb-1">Nama Kelas (Contoh: Bahasa Korea EPS TOPIK)</label>
                <input type="text" name="nama" id="nama" value="{{ old('nama') }}" required autofocus maxlength="20" class="shadow-sm focus:ring-primary-500 focus:border-primary-500 block w-full sm:text-sm border-slate-300 rounded-md">
