...
@section('header-sub', 'Perbarui detail ujian dan daftar soal')
...
<div class="card p-8" style="max-width: 800px; margin: 0 auto;">
    <form action="{{ route('admin.ujian.update', $ujian->id) }}" method="POST">
        ...
        <div class="mb-6">
            ...
            <textarea name="deskripsi" class="w-full px-4 py-3 border border-slate-200 rounded-lg outline-none focus:border-blue-500" placeholder="Tuliskan keterangan singkat mengenai ujian ini..." rows="3">{{ old('deskripsi', $ujian->deskripsi) }}</textarea>
        </div>

        <div class="grid-2 mb-6">
            <div>
                <label class="block text-sm font-bold mb-2 text-slate-700">Durasi (Menit)</label>
                <input type="number" name="durasi" class="w-full px-4 py-3 border border-slate-200 rounded-lg outline-none focus:border-blue-500" value="{{ old('durasi', $ujian->durasi) }}" required>
            </div>
            <div>
                ...
                <select name="kelas_id" class="w-full px-4 py-3 border border-slate-200 rounded-lg outline-none focus:border-blue-500" required>
                    ...
                    @foreach($kelases as $k)
                        <option value="{{ $k->id }}" {{ old('kelas_id', $ujian->kelas_id) == $k->id ? 'selected' : '' }}>{{ $k->nama }}</option>
                    @endforeach
                </select>
            </div>
        </div>
        ...
        <div class="grid-2 mb-8">
            <div>
                ...
                <input type="datetime-local" name="mulai" class="w-full px-4 py-3 border border-slate-200 rounded-lg outline-none focus:border-blue-500" value="{{ old('mulai', $ujian->mulai ? \Carbon\Carbon::parse($ujian->mulai)->format('Y-m-d\TH:i') : '') }}">
            </div>
            <div>
                <label class="block text-sm font-bold mb-2 text-slate-700">Waktu Berakhir</label>
                <input type="datetime-local" name="selesai" class="w-full px-4 py-3 border border-slate-200 rounded-lg outline-none focus:border-blue-500" value="{{ old('selesai', $ujian->selesai ? \Carbon\Carbon::parse($ujian->selesai)->format('Y-m-d\TH:i') : '') }}">
            </div>
        </div>
                        ...
                        <tbody>
                            ...
                            <tr class="soal-item" data-package-id="{{ $paket->id }}" style="border-bottom: 1px solid #f1f5f9; transition: background 0.2s;">
                                <td class="py-4 px-4 text-center" style="width: 50px;">
                                    <input type="checkbox" name="soal_ids[]" id="soal_{{ $s->id }}" value="{{ $s->id }}" 
                                        class="item_soal paket-id-{{ $paket->id }}" 
                                        {{ in_array($s->id, old('soal_ids', $selectedSoal)) ? 'checked' : '' }}
